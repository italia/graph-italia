...
export default function EditApiKeysPage() {
  ...
  const [expire, setExpire] = useState(60); // minutes
  ...
  const handleCreate = async () => {
    setIsCreating(true);
    try {
      const key = await api.createApiKey({ role, expire });
      if (key) {
        setNewKeyData(key);
        fetchKeys();
      }
                ...
                <table className="table w-full">
                  <thead>
                    <tr>
                      ...
                      <th>{t("table.expires", "Expires (m)")}</th>
                    ...
                    {keys.length === 0 ? (
                      ...
                      keys.map((key) => (
                        <tr key={key.id} className="hover">
                          <td className="font-mono text-xs">{key.id}</td>
                          <td>
                            <div className={`badge ${key.role === "READWRITE" ? "badge-secondary" : "badge-ghost"}`}>
                              {key.role}
                            </div>
                          </td>
