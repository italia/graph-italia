...
/** Index — returns dashboards for the user's default project */
router.get("/", async (c) => {
	try {
		...
		const results = await db.findDashboardsByProjectId(projectId);
...
/** Create — places dashboard in the user's default project */
router.post("/", requireUser, zValidator("json", createDashboardSchema), async (c) => {
	try {
		...
		logger.info("Dashboard created", { dashboardId: result.id, userId: user.userId });
...
router.delete("/:id", requireUser, zValidator("param", detailSchema), async (c) => {
	try {
		...
		const allowed = await db.canUserModifyProject(user.userId, dashboard.projectId);
...
/** Update slots */
router.put("/:id/slots", requireUser, zValidator("param", detailSchema), zValidator("json", updateSlotsSchema), async (c) => {
	try {
		const user = c.get("user") as ParsedToken;
		const { id: dashboardId } = c.req.valid("param");
...
/** Update :ID */
router.put("/:id", requireUser, zValidator("param", detailSchema), zValidator("json", updateDashboardSchema), async (c) => {
	try {
		const user = c.get("user") as ParsedToken;
		const { id: dashboardId } = c.req.valid("param");
		const dashboardData = c.req.valid("json");
